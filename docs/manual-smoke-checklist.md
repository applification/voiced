...
9. Revoke Accessibility, then dictate with Command + Shift + Space into an editor. Confirm the notch explains that insertion failed while the raw capture remains safely in Inbox. Revoke Microphone and confirm the notch reports that microphone access is needed.
...
- Hold Cmd+Shift+Space for 3–15 seconds. Words appear near the caret without focusing Voiced; the provisional tail changes. Release: final recognition inserts once. Test a quick press/release and Escape during setup/finalization.
...
- Cmd+Option+Shift+Space only saves to Inbox, in either mode. Verify normal Cmd shortcuts, Option+Space and typing spaces still work.
