...
private struct NotchTranscribingIcon: View {
    ...
    var body: some View {
        Image(systemName: "waveform")
            ...
            .scaleEffect(reduceMotion ? 1 : (pulse ? 1.16 : 0.92))
            .opacity(reduceMotion ? 1 : (pulse ? 1 : 0.58))
            .animation(
...
struct LevelWaveformView: View {
    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            let normalizedLevel = reduceMotion
                ? 0.42
                : (rawLevel == 0 ? 0.16 : max(0.16, pow(rawLevel, 0.5)))
            ...
            .animation(reduceMotion ? nil : .easeOut(duration: 0.045), value: level)
        }
    }
}
...
private struct LivePulseDot: View {
    ...
    var body: some View {
        Circle()
            ...
            .scaleEffect(reduceMotion ? 1 : (isPulsing ? 1.35 : 0.72))
            .opacity(reduceMotion ? 1 : (isPulsing ? 0.45 : 1))
            .animation(
                reduceMotion ? nil : .easeOut(duration: 0.62).repeatForever(autoreverses: true),
                value: isPulsing
            )
            .onAppear {
