...
Resolve one stable target, run independent assessments when possible, synthesize a design critique, and ask the user what to improve next. The chat response is the primary deliverable.

### Hard Invariants

- Assessment A (design review) and Assessment B (browser/source evidence) are both required.
- Assessment A must finish before source/browser evidence enters the parent synthesis context.
...
2. **Read `.impeccable/critique/ignore.md`** if it exists. Drop matching findings silently; it is the only prior-run input critique consumes.
...
### Assessment B: Browser + Source Evidence

Gather concrete evidence from source files, browser screenshots, accessibility/performance tooling already present in the repo, and manual inspection. Assessment B is mandatory and must remain isolated from Assessment A until both are complete.
...
2. Capture desktop and mobile screenshots when feasible.
3. Inspect console errors, keyboard focus, contrast risk, responsive behavior, loading/empty/error states, and interaction affordances.
4. Run existing project checks when they are cheap and relevant, such as lint, typecheck, accessibility tests, Lighthouse, or Playwright. Do not install Impeccable or call `npx`.
5. For multi-view targets, inspect 3-5 representative pages or states.

Codex Browser note: Use the Browser skill when available. Do not spend a Browser attempt on `file://` unless no better target exists.

Return: inspected files/routes, screenshot/viewports checked, tool output summaries, console/browser findings, likely false positives, and skipped/failed steps with concrete reasons.

Codex failure accounting: final Run Notes must include target, ignore list, assessment independence, browser visibility, source/tool checks, and any fallback signal used. Do not run repo status checks, late API spelunking, or unrelated verification after the report is assembled.

### Generate Combined Critique Report

Synthesize both assessments into a single report. Do NOT simply concatenate. Weave the findings together, noting where the design review and evidence pass agree, where the evidence pass caught issues the design review missed, and where tool findings are likely false positives.
...
**Evidence pass**: Summarize what the source/browser/tooling pass found, with file locations or routes. Note any additional issues it caught that you missed, and flag likely false positives.

**Visual inspection**: Summarize the viewports and states inspected. If browser inspection was unavailable, say what fallback signal you used instead.
...
#### Run Notes
Keep this compact. Include status for target, ignore list, assessment independence, browser visibility, source/tool checks, and fallback signal used. For failed or skipped steps, give the concrete observed reason.
