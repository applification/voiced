...
4. **Pull in any prior critique** (optional signal): If `.impeccable/critique/` exists, inspect the most relevant recent markdown for the same target and fold P0/P1 items into your polish list. Mention the snapshot path so the user sees what you read. If no relevant snapshot exists, continue without it. The critique is one input among many. Do your own pass either way.
...
- **Treat automation carefully**: Run available QA commands when they are relevant, fix their defects, but never cite a clean result as proof that the work is polished.
