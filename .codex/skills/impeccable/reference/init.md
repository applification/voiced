...
- **Browser route notes**: capture the dev server or page route future Codex Browser inspection should use.
...
Before asking questions, thoroughly scan the project to discover what you can. This single crawl feeds PRODUCT.md, DESIGN.md, and future Browser inspection, so be thorough once rather than re-scanning later:
...
## Step 6: Note Browser Iteration Path

This slim Codex skill does not include Impeccable live mode or helper scripts. If the project has code with HTML entries and a dev server, note the best route or command for normal Codex Browser inspection so future design work can open the right surface quickly. Do not write `.impeccable/live/config.json` and do not patch CSP for Impeccable live mode.
...
- **Iterate visually**: use Codex Browser with `$impeccable critique <page>`, `$impeccable polish <page>`, or a targeted command such as `$impeccable layout <component>`.
