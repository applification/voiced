...
metadata:
  short-description: Production-grade frontend design, critique, polish, and audit.
---

Designs and iterates production-grade frontend interfaces. Real working code, committed design choices, exceptional craft.

Project-local Codex bundle adapted from `pbakaus/impeccable`; keep `NOTICE.md` and `LICENSE` with this skill. This is the markdown-first skills core only: no npm package, no `npx`, and no vendored CLI/runtime scripts.

## Setup

You MUST do these steps before proceeding:

1. Read local context directly. Prefer `PRODUCT.md` and `DESIGN.md` when present, then inspect the relevant code/design files. This project intentionally does not keep Impeccable's context loader; Codex can read the files itself, and Plantry already has `DESIGN.md`.
...
5. **If the project is brand-new (no existing CSS tokens / theme / committed brand colors found in step 3)**, choose a brand seed color deliberately from the product context and compose the rest of the palette around it. Use OKLCH throughout. **Skip this step when committed brand colors already exist; identity-preservation wins.**
...
This slim Codex skill does not include Impeccable live mode, pin/unpin helpers, or detector scripts.

### Routing rules

1. **No argument**: the user is asking "what should I do?" Make the menu context-aware instead of static. Read `PRODUCT.md`/`DESIGN.md` if present, inspect git changes and obvious UI files, then lead with the **2-3 highest-value next commands** with one-line reasons, followed by the full menu grouped by category. **Never auto-run a command; the recommendation is a suggestion the user confirms.**

   Reason over the signals; there is no score to obey:
   - `DESIGN.md` missing while UI code exists → `document` (capture the visual system).
   - No obvious prior critique exists in `.impeccable/critique/` → for a set-up project with a real surface, offering `$impeccable critique <surface>` is a strong default.
   - A recent critique has a low score or non-zero P0/P1 items → `polish` (use that snapshot as backlog), or re-run `critique` if it looks stale.
   - Git changes point at one surface → scope `audit` or `polish` to those files specifically, naming them.
   - A running dev server or obvious viewable surface → recommend `critique`, `audit`, or `polish` with Browser inspection.
